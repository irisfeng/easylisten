...
type Playing = { slug: string; gender: VoiceGender } | null;
...
export default function Home() {
  ...
  const [voiceGender, setCurrentVoiceGender] = useState<VoiceGender>("f");
  const audioRef = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    const local = loadPrefs();
    setPrefs(local);
    setCurrentVoiceGender(local.voiceGender ?? "f");
  ...
  function toggleAudio(piece: Piece) {
    const same = playing?.slug === piece.slug && playing.gender === voiceGender;
    ...
    audioRef.current?.pause();
    const maleSlug = `${piece.slug}-m`;
    const folder =
      voiceGender === "m" && (audioManifest.slugs as string[]).includes(maleSlug)
        ? maleSlug
        : piece.slug;
    ...
    void audio.play().then(() => setPlaying({ slug: piece.slug, gender: voiceGender })).catch(() => {
      setPlaying(null);
    });
  }
        ...
        <ol>
          {latestPieces.map((piece, index) => (
            <IssueRow
              ...
              piece={piece}
              index={index}
              playing={playing?.slug === piece.slug && playing.gender === voiceGender}
